...
$code = trim($_GET['code'] ?? '');
if ($code === '') { header("Location: ../public/home.php"); exit(); }
$p = (new ProductManager($db))->find($code);
...
<head>
    ...
    <title>Modifier Produit &mdash; eShop</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
            ...
            --success: #4caf78; --danger: #e05252; --warning: #d4a843;
        ...
        .container { max-width: 660px; margin: 0 auto; padding: 52px 24px 80px; }
        .page-title { font-family: var(--font-head); font-size: 2rem; margin-bottom: 4px; }
...
<nav>
    ...
    <a href="<?= $base ?>/views/public/home.php" class="btn btn-outline">&larr; Catalogue</a>
</nav>

<div class="container">
    <h1 class="page-title">Modifier le produit</h1>
    <p class="page-sub">R&eacute;f. <strong style="color:var(--accent)"><?= htmlspecialchars($p->code) ?></strong></p>
    ...
    <div class="form-card">
        <form method="POST" enctype="multipart/form-data" action="<?= $base ?>/controllers/updateProductController.php">
            <input type="hidden" name="code" value="<?= htmlspecialchars($p->code) ?>">

            <div class="row2">
                <div class="field">
                    <label>Code</label>
                    <input value="<?= htmlspecialchars($p->code) ?>" disabled>
                </div>
                <div class="field">
            ...
            <div class="row2">
                <div class="field">
                    <label>Prix (DT) *</label>
                    <input name="price" type="number" step="0.01" min="0" value="<?= htmlspecialchars($p->price) ?>" required>
                </div>
                <div class="field">
                    <label>Stock</label>
                    <input name="stock" type="number" min="0" value="<?= htmlspecialchars($p->stock) ?>">
                </div>
            </div>
            ...
            <div class="field">
                ...
                <img class="img-preview"
                     src="../../public/uploads/products/<?= htmlspecialchars($p->image) ?>"
                     alt="Image actuelle"
                     onerror="this.src='https://placehold.co/600x180/1c1c1e/7a7672?text=No+Image'">
                <label>Changer l'image <span style="color:var(--muted)">(optionnel)</span></label>
                <input type="file" name="image" accept="image/jpeg,image/png,image/gif,image/webp">
                <div class="hint">Laisser vide pour conserver l'image actuelle.</div>
            </div>

            <div class="submit-row">
                <a href="<?= $base ?>/views/public/home.php" class="btn btn-outline">Annuler</a>
                <button class="btn btn-warn" type="submit">&#9998; Mettre &agrave; jour</button>
            </div>
        </form>
    </div>
</div>
